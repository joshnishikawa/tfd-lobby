...
class GameLoader {
  ...
  loadAll() {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const isEnabled = this.isGameEnabled(config, dirName, manifest.id);

        let gameDef = null;
        const candidates = [
          path.join(gameDir, 'game.cjs'),
          path.join(gameDir, 'game.js'),
          path.join(gameDir, 'index.cjs'),
        ...
        for (const candidate of candidates) {
          if (fs.existsSync(candidate)) {
            try {
              gameDef = require(candidate);
              if (gameDef) break;
            } catch (loadErr) {
              console.error(`[GAME-LOADER] Error requiring ${candidate}:`, loadErr.message);
            }
          }
        }

        if (!gameDef) {
          console.warn(`[GAME-LOADER] Skipping ${manifest.name || dirName}: No valid game definition found (game.cjs, game.js, index.js)`);
          continue;
        }
        ...
        this.gameMetadata.push({
          ...
          devTools: manifest.devTools || manifest.tools || manifest.dev_tools || [],
          testModes: manifest.testModes || manifest.test_modes || [],
          enabled: isEnabled
        });

        console.log(`[GAME-LOADER] Successfully loaded game: ${manifest.name} (${manifest.id || game.name})${isEnabled ? '' : ' [Config: Disabled for regular users]'}`);
      } catch (err) {
  ...
  getMetadata({ isAdmin = false } = {}) {
    const config = this.loadConfig();
    return this.gameMetadata.map(meta => {
      const enabled = this.isGameEnabled(config, meta.dirName || meta.id, meta.id);
      if (!isAdmin) {
        const { devTools, testModes, ...safeMeta } = meta;
        return {
          ...safeMeta,
          enabled
        };
    ...
    }).filter(meta => isAdmin || meta.enabled !== false);
  }
}
