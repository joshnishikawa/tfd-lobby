...
class GroupManager {
  ...
  launchGame(code, memberId, matchId, resetPlayAgain = false) {
    const group = this.getGroup(code);
    if (!group) throw new Error('Group room not found');
    const member = group.members.find(m => m.id === memberId);
    if (!member) {
    ...
    if (resetPlayAgain) {
      // In 2-player rematches, alternate turn order so the player who went second last time goes first
      if (group.members.length === 2) {
        group.members.reverse();
      }
      group.members.forEach((m, idx) => {
        m.playerSeat = String(idx);
        m.playAgain = false;
        m.isReady = true;
      });
    }
